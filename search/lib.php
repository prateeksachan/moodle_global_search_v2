...
function search_get_iterators($requireconfig = TRUE) {
    ...
    $gs_course = 'gs_support_course';
    $course = 'course';
    if (!empty($CFG->$gs_course)) {
        $functions[$course] = new stdClass();
        $functions[$course]->iterator = $course . '_search_iterator';
        $functions[$course]->documents = $course . '_search_get_documents';
        $functions[$course]->access = $course . '_search_access';
